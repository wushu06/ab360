...
class Yamm_Nav_Walker extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		if ($item->is_dropdown && ($depth === 0)) {
			$item_html = str_replace('<a class="no-barba"', '<a class="dropdown-toggle" data-toggle="dropdown"', $item_html);
			$item_html = str_replace('</a>', ' <b class="caret"></b></a>', $item_html);
		} elseif (stristr($item_html, 'li class="divider')) {
			$item_html = preg_replace('/<a class="no-barba" [^>]*>.*?<\/a>/iU', '', $item_html);
		} elseif (stristr($item_html, 'li class="dropdown-header')) {
			$item_html = preg_replace('/<a class="no-barba"[^>]*>(.*)<\/a>/iU', '$1', $item_html);
		}
...
function Yamm_Nav_Walker_menu_fallback()
{
	...
	if (current_user_can('manage_options')) {
		$fb_output .= '<li><a  class="" href="' . admin_url('nav-menus.php') . '">' . __('Add a menu', THEMETD) . '</a></li>';
	}
	$fb_output .= '</ul>';
...
{
	$fb_output = '<ul class="nav navbar-nav">';
	$fb_output .= '<li><a class="this" href="' . esc_url(home_url()) . '">' . __('Home', THEMETD) . '</a></li>';
	if (current_user_can('manage_options')) {
